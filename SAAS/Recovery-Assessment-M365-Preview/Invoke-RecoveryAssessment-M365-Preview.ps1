...
.SYNOPSIS
    ...
    live-tenant assessment as Invoke-RecoveryAssessment-M365-Full.ps1 (tiers
    Mailboxes, OneDrive accounts, SharePoint sites, and Teams into Critical
    Group 1/2/3/Group 4 based on Microsoft Graph usage activity, then models
    recovery time and downtime cost per
    group), but is meant for webinars/trade shows: Group 1 is shown in full
    (real identity + real ABR timing) as a "here's what you'd actually see"
    proof point; Groups 2/3/4 keep their real object counts and Mass Recovery
    baseline timing but have every per-object identity field (name,
    identifier, job title, department, manager, criteria) redacted, and
    Groups 2/3 have their ABR-specific timing/savings withheld - all done
    server-side, before anything is written to a CSV or embedded in the HTML
    report, never via CSS/JS hiding. Scoring weights and manual overrides are
...
param(
    ...
    [string]$OutputPath = ".\M365CriticalityAssessmentPreview_$(Get-Date -Format 'yyyyMMdd_HHmmss')",
...
function Add-RecentDataEstimate {
    <#
        ...
        uniformly for all four workloads. This function only feeds
...
function Add-BudgetTier {
    <#
        ...
        Protect-PreviewData's redaction decision here and the exported
        CSVs/manifest - both would silently drift from what the live report
        ends up showing on load if this algorithm ever diverges from
        assignBudgetTiers(). See RECOVERY-MODEL-METHODOLOGY.md.
...
# NEW in the PREVIEW build only (not present in the full script): server-side
# redaction, run BEFORE Export-WorkloadResult writes any CSV and BEFORE
# New-M365HtmlReport embeds anything into the HTML - so a redacted row is
# redacted everywhere this script produces output, not just hidden by CSS/JS
# (which would still be fully readable via page source/dev tools). Per
# feedback 2026-07-30: this script runs against a REAL, LIVE prospect tenant
# at webinars/trade shows as a preview of the full assessment - Group 1 stays
# completely real (identity + timing, the "here's what you'd actually see"
# proof point), Groups 2/3/4 keep their real aggregate counts and Mass
# Recovery baseline timing (the scale of the problem), but every per-object
# IDENTITY field is replaced with a generic placeholder before it's ever
# written to disk. Numeric metrics (scores, activity counts, storage, item
# counts) are left untouched on every row so totals, tiers, and recovery-time
# math stay exactly correct - only identity is redacted, nothing numeric.
function Protect-PreviewData {
    param(
        [Parameter(Mandatory)] [array] $Data,
        [Parameter(Mandatory)] [string] $WorkloadLabel
    )
    $tierShort = @{
        'Critical Group 1' = 'Group 1'
        'Critical Group 2' = 'Group 2'
        'Critical Group 3' = 'Group 3'
        'Group 4'           = 'Group 4'
    }
    $identityFields = @('ObjectName', 'JobTitle', 'Department', 'Manager', 'OfficeLocation', 'EmployeeType')
    $counter = 0
    foreach ($row in $Data) {
        if ($row.Tier -eq 'Critical Group 1') { continue }
        $counter++
        # Excluded (untiered) rows never went through Add-Tier, so .Tier can be
        # $null here - Hashtable.ContainsKey($null) throws (ArgumentNullException,
        # not a graceful "not found"), so the null check has to come first.
        $shortTier = if (-not $row.Tier) { 'Excluded' } elseif ($tierShort.ContainsKey($row.Tier)) { $tierShort[$row.Tier] } else { $row.Tier }
        $placeholder = "$shortTier $WorkloadLabel #$counter"
        foreach ($field in $identityFields) {
            if ($row.PSObject.Properties.Name -contains $field) {
                Add-Member -InputObject $row -NotePropertyName $field -NotePropertyValue $placeholder -Force
            }
        }
        if ($row.PSObject.Properties.Name -contains 'Identifier') {
            Add-Member -InputObject $row -NotePropertyName 'Identifier' -NotePropertyValue '(redacted - see full assessment)' -Force
        }
        if ($row.PSObject.Properties.Name -contains 'ManagerChain') {
            Add-Member -InputObject $row -NotePropertyName 'ManagerChain' -NotePropertyValue @() -Force
        }
        # NEW (-Groups): Entra ID group membership is identity-adjacent data
        # (which security/distribution groups a real person belongs to) -
        # blanked for every non-Group-1 row exactly like ManagerChain above,
        # never shipped to the browser for a redacted object.
        if ($row.PSObject.Properties.Name -contains 'Groups') {
            Add-Member -InputObject $row -NotePropertyName 'Groups' -NotePropertyValue @() -Force
        }
        if ($row.PSObject.Properties.Name -contains 'GroupIds') {
            Add-Member -InputObject $row -NotePropertyName 'GroupIds' -NotePropertyValue @() -Force
        }
        if ($row.PSObject.Properties.Name -contains 'CriteriaTags') {
            Add-Member -InputObject $row -NotePropertyName 'CriteriaTags' -NotePropertyValue 'Full criteria detail in the complete assessment' -Force
        }
        if ($row.PSObject.Properties.Name -contains 'TitleWeightMatch') {
            Add-Member -InputObject $row -NotePropertyName 'TitleWeightMatch' -NotePropertyValue '' -Force
        }
        if ($row.PSObject.Properties.Name -contains 'HubSiteKeywordMatch') {
            Add-Member -InputObject $row -NotePropertyName 'HubSiteKeywordMatch' -NotePropertyValue '' -Force
        }
    }
    return $Data
}
...
function Get-MailboxCriticality {
    ...
    $rows = foreach ($u in $usage) {
        ...
        # -DetailedSizing (Full-only) - and is intentionally NOT gathered here.
...
/* NEW: Recovery Ladder - single to-scale horizontal timeline for the lean
   ...
   PREVIEW-ONLY (v3.10.2), per feedback 2026-08-12: this no longer mirrors
   the Full script. Group 1 stays fully real (position, color, time) and the
   whole-tenant Mass Recovery baseline stays real (same aggregate-figure
   exception as everywhere else in Preview), but Groups 2/3's own cumulative
   times and track geometry are now locked - see buildRecoveryLadderHtml. */
...
/* PREVIEW-ONLY (v3.10.2): the single undifferentiated region covering
   everything after Group 1 - no internal boundary, so Group 2/3's relative
   proportions never leak through segment geometry even with their time
   values blurred. Same diagonal-stripe language as .pv-locked-panel. */
.rl-seg.locked { background-color: #EEF2F6; background-image: repeating-linear-gradient(135deg, #EEF2F6 0, #EEF2F6 8px, #E2E8EE 8px, #E2E8EE 16px); }
...
.rl-flag .rl-pin.locked { background: #C7D0DA; }
...
/* NEW: Sizing tab - Exchange/OneDrive/SharePoint/Discovery Summary tables,
   ...
   rendered once at bootstrap (see recomputeAll). Mirrors Full script
   exactly; built only from aggregate metrics + RecipientType/HasArchive,
   none of which Protect-PreviewData redacts (identity fields only), so no
   PREVIEW-ONLY changes are needed here. */
...
/* NEW: scoring-controls redesign - percentage weights (was raw 0-1 decimals)
   ...
   In this Preview build the sliders stay disabled/frozen (unchanged from
   before) and the Live Impact Preview only ever shows Group 1 numbers -
   Groups 2/3/4 are pvLocked() exactly like every other panel in this file. */
...
/* PREVIEW-ONLY (v3.7.5, relaxed v3.7.6/v3.7.8): locked/redacted display for
   anything beyond Group 1 or the whole-tenant Mass Recovery baseline - see
   pvLocked()/pvLockedPanel() in ReportJsEngine. The placeholder text itself
   is blurred (never the real value, so it isn't a page-source leak like
   blurring real digits would be) for a visibly "redacted" look, plus a
   small tag explaining why. pv-locked-panel replaces an entire section
   (Group 4 Detail) that has no Group-1-only content to fall back to - it's
   a static placeholder, not real data, so it uses a decorative frosted
   pattern rather than a blur filter. */
.pv-locked-value { display: inline-block; filter: blur(4px); user-select: none; -webkit-user-select: none; }
.pv-locked-tag { display: inline-block; font-size: .6rem; font-weight: 700; letter-spacing: .3px; text-transform: uppercase; color: var(--blue); margin-left: .4rem; white-space: nowrap; vertical-align: middle; }
.pv-locked-tag::before { content: "\1F512"; margin-right: .2rem; }
.pv-locked-panel { border-radius: 12px; padding: 1.8rem 1.4rem; text-align: center; background: repeating-linear-gradient(135deg, #EEF2F6 0, #EEF2F6 12px, #E2E8EE 12px, #E2E8EE 24px); border: 1px dashed #C7D0DA; }
.pv-locked-panel-icon { font-size: 1.5rem; margin-bottom: .3rem; opacity: .7; }
.pv-locked-panel-title { font-weight: 800; color: var(--navy); font-size: .92rem; }
.pv-locked-panel-note { font-size: .78rem; color: var(--dark-gray); margin-top: .35rem; max-width: 460px; margin-left: auto; margin-right: auto; }
.pv-locked-mini { display: inline-block; padding: .1rem .4rem; border-radius: 4px; background: #EEF2F6; }
...
// NEW v3.7.4: Enterprise Applications removed entirely, per feedback that it
// wasn't needed for the SaaS data-protection story this tool tells. PREVIEW
// mode's scope footprint shrinks to Reports.Read.All only (AuditLog.Read.All
// existed solely to pull sign-in logs for this workload).
var WORKLOAD_DEFS = [
...
function computeScoresAndTiers(workloadKey) {
  ...
  // v3.1.0: base population split is criticality-ranked, ensuring every
  // group gets a population as long as there's enough active data. See
  // computeRecoveryModel for the separate ABR (hot-scope) vs Mass Recovery
  // (full-scope) recovery-TIME calculation.
...
// PREVIEW-ONLY (v3.7.5): per feedback 2026-07-31 - Group 1 is the only tier
// this build ever shows real figures for. Everything about Groups 2/3/4 (or
// whole-tenant aggregates that fold them in) used to still show real object
// counts and Mass Recovery timing - "the scale of the problem" - with only
// per-object IDENTITY redacted. That's now considered too much to reveal at
// a live demo: pvLocked() masks a value with a fixed placeholder (never the
// real number - unlike a CSS blur over real text, it can't be recovered via
// view-source/inspect) and a small blurred "Full assessment" tag. Used for
// any count/time/cost/percentage that isn't Group-1-only or the whole-tenant
// Mass Recovery baseline (both of those stay real - see v3.7.6/v3.7.8).
// pvLockedPanel() is the equivalent for an entire section (Group 4 Detail)
// that has no Group-1-only content to fall back to.
function pvLocked(placeholder) {
  return '<span class="pv-locked-value">' + (placeholder || "••••") + '</span><span class="pv-locked-tag">Full assessment</span>';
}
function pvLockedPanel(title, note) {
  return '<div class="pv-locked-panel"><div class="pv-locked-panel-icon">&#128274;</div>' +
    '<div class="pv-locked-panel-title">' + esc(title) + '</div>' +
    '<div class="pv-locked-panel-note">' + esc(note || "Available in the full assessment - connect with your Rubrik team.") + '</div></div>';
}
'@
...
function renderExecFinancialAndRecoveryTop() {
  ...
  // objects to a 5th bucket. PREVIEW-ONLY (v3.7.5): only reports on Group 1's
  // OWN flag now - whether Groups 2/3 individually exceed their targets is
  // real information about this tenant beyond Group 1, so it's withheld.
  var g1ExceedsOwnTarget = g1.exceedsTarget;
  var targetGapNote = g1ExceedsOwnTarget
    ? ("<span style=\"color:var(--red-target);font-weight:700;\">Group 1 exceeds its stated RTO target</span> by " + fmtMin(g1.targetGapMin) + ".")
    : "Group 1's ABR recovery time is within its stated RTO target.";
  ...
  if (model.fullRestoreMin > 0) {
    // PREVIEW-ONLY (v3.7.6): the whole-tenant Mass Recovery baseline ("instead
    // of Y") is real again, per feedback 2026-07-31 - it's a single aggregate
    // figure for the ENTIRE tenant used purely as the "before" side of the
    // Group-1 comparison, not a breakdown of Groups 2/3/4 individually, so it
    // doesn't expose anything about how this tenant's non-Group-1 data is
    // split. Per-group figures (Groups 1-2/1-3, Group 4 detail) stay locked.
    financialHtml += '<div class="exec-fin-headline">Your most critical data is usable in <b>' + fmtMin(g1.wallClockCumMin) + '</b> instead of <b>' + fmtMin(model.fullRestoreMin) + '</b> with a traditional, random-order restore.</div>';
  }
  ...
  if (state.recovery.costPerHour > 0) {
    // PREVIEW-ONLY (v3.7.8, relaxed from v3.7.6 per feedback 2026-07-31): the
    // Recovery Timeline graphic is real again, same reasoning as the
    // whole-tenant Mass Recovery baseline it sits alongside - it's Group
    // 1/2/3 shown together against the whole-tenant Mass Recovery bar as one
    // "big picture" comparison, not a way to isolate any single group's own
    // figures in confidence.
    financialHtml +=
      ...
      "</div>" +
      '<div class="exec-fin-punchline">That\'s ' + fmtMoney(g1SavingsMoney) + " in downtime cost avoided (" + pctReduction + "% reduction) by getting Group 1 online first - Mass Recovery keeps running in the background for everything outside Group 1.</div>";
  ...
  // PREVIEW-ONLY (v3.7.5): Group 4 Detail used to still show real
  // percentages/counts for every workload - per feedback 2026-07-31, that's
  // whole-tenant scale information beyond Group 1, so it's locked to a
  // static panel now instead of a collapsible with real numbers behind it.
  var dormantHtml = totalObjectsAll > 0
    ? ('<div style="margin-top:1.8rem;">' + pvLockedPanel("Group 4 Detail", "See the full zero-activity breakdown, by workload, in the complete assessment.") + "</div>")
    : "";

  // Group 1's own object count is real; Groups 2/3's counts (and whether
  // they're empty) are locked below, same as everywhere else on this tab.
  ...
  var recoveryHtml = '<div class="exec-hero-label">Recovery Times (ABR, sequenced by group)</div>' +
    ...
    '<div class="exec-hero-grid">' +
      ...
      '<div class="exec-hero-card"><div class="exec-hero-tag">Groups 1-3 online in</div><div class="exec-hero-value">' + pvLocked() + '</div><div class="exec-hero-sub">cumulative - target: ' + state.recovery.group3Hours + " hr</div></div>" +
      '<div class="exec-hero-card"><div class="exec-hero-tag">Full tenant, no prioritization</div><div class="exec-hero-value">' + fmtMin(model.fullRestoreMin) + '</div><div class="exec-hero-sub">Mass Recovery baseline for comparison</div></div>' +
    "</div>" +
  ...
  // 1-2, and 1-3 cumulative), not just Group 1. PREVIEW-ONLY (v3.7.5): rows
  // beyond Group 1 (and the full-tenant baseline row) are locked - each of
  // Time/Downtime Cost/Cost Avoided/Reduction would otherwise reveal
  // whole-tenant scale beyond Group 1.
  var costTableHtml = "";
  if (state.recovery.costPerHour > 0 && model.fullRestoreMin > 0) {
    var g1Cost = fmtHours(g1.wallClockCumMin) * state.recovery.costPerHour;
    // PREVIEW-ONLY (v3.7.6): the whole-tenant baseline row is real again
    // (single aggregate figure, same reasoning as the headline/hero-grid
    // above) - only the Groups 1-2/1-3 rows stay locked, since those are the
    // ones that would expose Group 2/3-specific scale.
    var costRows = "<tr><td>Group 1 online</td><td>" + fmtMin(g1.wallClockCumMin) + "</td><td>" + fmtMoney(g1Cost) + "</td><td colspan=\"2\">See the full assessment for cost-avoided/reduction figures</td></tr>" +
      ["Groups 1-2 online", "Groups 1-3 online"].map(function (label) {
        return "<tr><td>" + label + "</td><td>" + pvLocked() + "</td><td>" + pvLocked() + "</td><td>" + pvLocked() + "</td><td>" + pvLocked() + "</td></tr>";
      }).join("") +
      "<tr><td>Full tenant (Mass Recovery baseline)</td><td>" + fmtMin(model.fullRestoreMin) + "</td><td>" + fmtMoney(fullExposureMoney) + "</td><td colspan=\"2\">Baseline - no prioritization applied</td></tr>";
...
// Recovery Ladder - single to-scale horizontal timeline for the lean
...
// PREVIEW-ONLY (v3.10.2), per feedback 2026-08-12: this used to mirror the
// Full script exactly (real time + true-to-scale position for every
// milestone). That leaked whole-tenant scale beyond Group 1 - a viewer
// could read Business-Important/Standard's real cumulative hours straight
// off the labels, and even without the labels, the segment widths and pin
// positions revealed their relative proportions geometrically. Now: only
// Critical Group 1 (already real everywhere else) and the whole-tenant Mass
// Recovery baseline (already a real, single aggregate figure everywhere
// else) keep their true time/position. Everything from the end of Group 1
// to the right edge collapses into ONE undifferentiated locked region -
// no internal boundary at all - so Group 2/3's real cumMin is used only to
// decide whether their pin/label appears (i.e. whether that tier has any
// population), never to place or size anything. Their pins are spread
// evenly across the locked region instead, and their time value uses
// pvLocked() - the same blur-plus-tag treatment as the Groups 1-2/1-3 rows
// in the hero grid and cost table above.
function buildRecoveryLadderHtml(model) {
  ...
  var g1 = milestones.filter(function (m) { return m.tier === "Critical Group 1"; })[0];
  var lockedMilestones = milestones.filter(function (m) { return m.tier !== "Critical Group 1"; });
  var g1TruePct = g1 ? Math.max(0, Math.min(100, (g1.cumMin / full) * 100)) : 0;
  var lockedStart = g1 ? g1TruePct : 0;
  var lockedSpan = Math.max(0, 100 - lockedStart);

  var segHtml = (g1 ? '<div class="rl-seg" style="width:' + g1TruePct + '%;background:' + g1.color + ';"></div>' : "") +
    '<div class="rl-seg locked" style="width:' + lockedSpan + '%;"></div>';

  var flagsHtml = g1
    ? ('<div class="rl-flag" style="left:' + Math.max(g1TruePct, 6) + '%;">' +
        '<div class="rl-time">' + fmtMin(g1.cumMin) + '</div>' +
        '<div class="rl-pin" style="background:' + g1.color + ';"></div>' +
        '<div class="rl-label">' + esc(g1.short) + '<br>' + esc(g1.verb) + '</div>' +
      '</div>')
    : "";

  flagsHtml += lockedMilestones.map(function (m, i) {
    var pos = Math.min(lockedStart + (lockedSpan * (i + 1)) / (lockedMilestones.length + 1), 92);
    return '<div class="rl-flag" style="left:' + pos + '%;">' +
      '<div class="rl-time">' + pvLocked() + '</div>' +
      '<div class="rl-pin locked"></div>' +
...
function renderExecLean() {
  ...
  var html;
  if (model.fullRestoreMin > 0) {
    // PREVIEW-ONLY (v3.7.6): both tiles here are real again, per feedback
    // 2026-07-31 - the whole-tenant Mass Recovery baseline and the $ saved/%
    // reduction derived from it are single aggregate figures relative to
    // Group 1, not a Groups-2/3/4 breakdown, so they don't need to stay
    // locked. Group 1's own time was already real.
    var moneyTileHtml = '<div class="exec-lean-tile money"><div class="exec-lean-tile-label">Downtime Cost Avoided</div><div class="exec-lean-tile-value">' + fmtMoney(savingsMoney) + '</div><div class="exec-lean-tile-sub">' + pctReduction + '% reduction vs. a traditional, random-order restore</div></div>';
    ...
    html = '<div class="exec-lean-tiles">' +
        '<div class="exec-lean-tile"><div class="exec-lean-tile-label">Time to Critical Data</div><div class="exec-lean-tile-value">' + fmtMin(g1.wallClockCumMin) + '</div><div class="exec-lean-tile-sub">instead of ' + fmtMin(model.fullRestoreMin) + ' with a traditional, random-order restore</div></div>' +
        moneyTileHtml +
      "</div>" +
...
function renderExecSummary() {
  ...
  var cardsHtml = "";
  var group1Tier = TIER_ORDER[0];
  var grandGroup1Total = 0;

  // PREVIEW-ONLY (v3.7.5): a per-workload card's TOTAL (all tiers summed)
  // and its tier-bar/legend both reveal whole-tenant scale beyond Group 1 -
  // per feedback 2026-07-31, only Group 1's own count is shown per workload
  // now; no workload total, no proportional bar, no per-tier legend beyond
  // Group 1.
  WORKLOAD_DEFS.forEach(function (wd) {
    var rows = DATA.workloads[wd.key] || [];
    var g1Count = rows.filter(function (r) { return r.Tier === group1Tier; }).length;
    grandGroup1Total += g1Count;
    cardsHtml += '<div class="summary-card" style="--accent:var(--blue);" onclick="goToWorkload(\'' + wd.key + '\')">' +
      "<h3>" + esc(wd.label) + '</h3><div class="total">' + fmtNum(g1Count) + '</div><div class="tier-legend"><div><span style="width:8px;height:8px;border-radius:50%;background:' + TIER_META[group1Tier].bg + ';display:inline-block;"></span>Group 1 objects</div></div>' +
      '<div class="pv-locked-mini" style="margin-top:.4rem;font-size:.72rem;color:var(--dark-gray);">Groups 2-4: full breakdown in the complete assessment</div></div>';
  });
  document.getElementById("exec-summary-cards").innerHTML = cardsHtml;

  // PREVIEW-ONLY (v3.7.5): only the Group 1 tile is real - the other three
  // used to show real cross-workload totals ("scale of the problem"),
  // locked now for the same reason as everywhere else in this build.
  var totalsHtml = '<div class="totals-grid">';
  TIER_ORDER.forEach(function (t) {
    var valueHtml = t === group1Tier ? fmtNum(grandGroup1Total) : pvLocked();
    totalsHtml += '<div class="totals-cell"><div class="label">' + TIER_META[t].short + ' - Objects</div><div class="value">' + valueHtml + "</div></div>";
  });
  totalsHtml += "</div>";
...
function buildWorkloadTotalsRow(wdKey, rowsOverride, headingLabel) {
  ...
  // PREVIEW-ONLY (v3.7.5): only the Group 1 cell is real - the other tiers
  // used to show real object/item/storage totals for this workload ("scale
  // of the problem"), locked now per feedback 2026-07-31.
  ...
  TIER_ORDER.forEach(function (t) {
    if (t === TIER_ORDER[0]) {
      ...
      html += '<div class="totals-cell"><div class="label">' + TIER_META[t].short + '</div><div class="value">' + pvLocked() + "</div></div>";
    }
  });
  html += "</div>";
  return html;
...
function buildWorkloadSection(wdKey) {
  ...
  var chips = "";
  // PREVIEW-ONLY (v3.7.5): the count next to each chip used to be real for
  // every tier (and for "All", which sums all four) - locked now for
  // anything except Group 1, since a real count is real tenant-scale data.
  // The chip itself stays clickable (a presenter can still show what a
  // redacted Group 2/3/4 row looks like) - only the number is hidden.
  ["all"].concat(TIER_ORDER).forEach(function (t) {
    var label = t === "all" ? "All" : TIER_META[t].short;
    var activeClass = ((f.tier || "all") === t) ? " active" : "";
    var countHtml = t === TIER_ORDER[0] ? String(rows.filter(function (r) { return r.Tier === t; }).length) : "&bull;&bull;&bull;";
    chips += '<button class="chip' + activeClass + '" onclick="setFilter(\'' + wdKey + '\',\'tier\',\'' + t + '\')">' + label + " (" + countHtml + ")</button>";
  });
  ...
  // PREVIEW-ONLY (v3.7.5): a real "X of Y shown" count is only safe to
  // reveal when every visible row is Group 1 (Y otherwise = whole-workload
  // total, which is locked everywhere else in this build). buildWorkloadTotalsRow
  // itself already locks every non-Group-1 cell regardless of what subset
  // it's given, so this only affects the heading text above it.
  var isGroup1Only = visibleRows.length > 0 && visibleRows.every(function (r) { return r.Tier === TIER_ORDER[0]; });
  ...
  // PREVIEW-ONLY (v3.7.5): same isGroup1Only reasoning as the filtered-totals
  // heading above - a section header count is only ever shown when every
  // visible row is Group 1 (real, small, safe); otherwise locked.
  var headerCount = isGroup1Only ? (fmtNum(visibleRows.length) + " shown") : pvLocked();
...
// PREVIEW-ONLY (v3.7.2): no tier dropdown, no override badge/UI - the full
// script's per-row manual override is a tiering-affecting control, frozen
// here for the same reason the weight sliders are (see buildControlsPanel).
// Tier shows as a plain read-only badge instead.
// PREVIEW-ONLY (v3.7.5): Protect-PreviewData (server-side) already replaces
// identity fields (ObjectName/Identifier/JobTitle/Department/Manager/
// CriteriaTags) with a placeholder for every non-Group-1 row - but Score,
// Last Activity, and the item/storage metrics were still real, per-object
// numbers. Per feedback 2026-07-31, those are locked here too for any row
// that isn't Group 1, so a redacted row never still hands over real,
// individually-readable metrics.
//
// IMPORTANT (found during verification of the above): Add-Tier (server,
// decides what Protect-PreviewData redacts) and computeScoresAndTiers
// (client, decides what tier badge actually renders) are two independent
// implementations of the same algorithm - a real tenant run surfaced a
// mailbox at a tier boundary where they disagreed (server said Group 1 so
// identity was left real; client recomputed Group 2). Since identity is
// baked into the page as plain JSON, the client is the only remaining place
// that can catch this - so ObjectName/Identifier/CriteriaTags are ALSO
// re-masked here whenever isGroup1Row is false, regardless of what the raw
// field actually contains. This is a second, independent check, not a
// replacement for the server-side redaction.
function buildRowHtml(row, wd, wdKey, hasEnrichment, hasMailboxType, hasGroups) {
  ...
  var isGroup1Row = row.Tier === TIER_ORDER[0];
  var pct = Math.round((row._Score || 0) * 100);
  var extra = "";
  if (hasEnrichment) {
    extra += '<td class="col-detail">' + (isGroup1Row ? esc(row.JobTitle) : "") + '</td><td class="col-detail">' + (isGroup1Row ? esc(row.Department) : "") + '</td><td class="col-detail">' + (isGroup1Row ? esc(row.Manager) : "") + "</td>";
  }
  if (hasGroups) { extra += '<td class="col-detail">' + (isGroup1Row ? esc((row.Groups || []).join(", ")) : "") + "</td>"; }
  if (hasMailboxType) { extra += '<td class="col-detail">' + (isGroup1Row ? esc(row.MailboxTypeHeuristic) : pvLocked()) + "</td>"; }
  return "<tr>" +
    "<td>" + (isGroup1Row ? esc(row.ObjectName) : "(redacted)") + "</td>" +
    "<td>" + (isGroup1Row ? esc(row.Identifier) : "(redacted - see full assessment)") + "</td>" +
    '<td><span class="badge" style="background:' + tierMeta.bg + ";color:" + tierMeta.fg + ';">' + tierMeta.short + "</span></td>" +
    (isGroup1Row
      ? ('<td><div class="score-bar-wrap"><div class="score-bar"><span style="width:' + pct + '%;"></span></div>' + pct + "%</div></td>")
      : ("<td>" + pvLocked() + "</td>")) +
    '<td class="criteria-tags col-detail">' + (isGroup1Row ? esc(row.CriteriaTags) : "Full criteria detail in the complete assessment") + "</td>" +
    "<td>" + (isGroup1Row ? esc(row.LastActivityDate) : pvLocked()) + "</td>" +
    "<td>" + (isGroup1Row ? fmtNum(row.metrics[wd.itemField]) : pvLocked()) + "</td>" +
    (wd.storageField ? ("<td>" + (isGroup1Row ? (parseFloat(row.metrics[wd.storageField]) || 0).toFixed(2) : pvLocked()) + "</td>") : "") +
    extra + "</tr>";
}
...
// PREVIEW-ONLY (v3.7.2): the full script's live weight sliders, title-weight
// keyword table, and hub-site keyword editor are removed here - per feedback
// 2026-07-30, this build is a locked preview of Group 1 (downtime-cost
// controls stay live; see buildRecoveryControls) rather than a fully tunable
// instance. Changing any of those inputs would change which objects land in
// which GROUP, and a redacted Group 2/3/4 object silently moving into the
// fully-real Group 1 view is exactly the failure mode this build cannot
// allow - so the values are shown read-only instead of removed outright,
// to keep the "why" for Group 1's composite score fully transparent.
// NEW: same percentage display as the Full assessment (was a raw 0-1
// decimal) - but every slider here stays disabled, exactly as before. No
// oninput handler is wired up (there is nothing to clamp when nothing can
// move), so onWeightSlide()/refreshWeightTotal() from the Full build are
...
function buildControlsSliders() {
  ...
  WORKLOAD_DEFS.filter(function (w) { return w.spExOd !== null || w.key === "teams"; }).forEach(function (wd) {
    ...
    var sumPct = 0;
    Object.keys(weights).forEach(function (field) { sumPct += Math.round(weights[field] * 100); });
    html += '<div class="wd-block"><div class="wd-head"><b>' + esc(wd.label) + '</b><span class="wd-total ok">= ' + sumPct + '%</span></div>';
    Object.keys(weights).forEach(function (field) {
      var pct = Math.round(weights[field] * 100);
      html += '<div class="control-row"><label>' + (FRIENDLY[field] || field) + '</label><input type="range" min="0" max="100" step="5" value="' + pct + '" disabled><span class="valdisp">' + pct + "%</span></div>";
    });
    html += "</div>";
  });
  html += '<div class="bonus-section"><div class="bonus-label-row"><b style="font-size:.8rem;color:var(--navy);">Bonus modifiers</b><span class="bonus-tag">Fixed in this preview</span></div>';
...
// Static snapshot only - Group 1 is real (as everywhere else in this
// build); Groups 2/3/4 counts and bars are pvLocked() so nothing beyond
// Group 1 and whole-tenant aggregates is ever real in this preview, matching
// every other panel here. Since weights are frozen there's no live
// recompute/churn to show, so this intentionally has no +N/-N delta chips
// or "Live" badge - it's a one-time breakdown, not a live preview.
function buildGroupBreakdownPreview() {
  var html = '<div class="preview-card"><div class="preview-head"><h5>Group Breakdown</h5></div>' +
    '<p class="preview-sub">Group 1 reflects this tenant\'s real data. Groups 2-4 are locked in this preview - connect with your Rubrik team for the full breakdown.</p>';
  TIER_ORDER.forEach(function (tier) {
    var isG1 = tier === "Critical Group 1";
    var groupTotal = 0;
    WORKLOAD_DEFS.forEach(function (wd) {
      var rows = DATA.workloads[wd.key] || [];
      groupTotal += rows.filter(function (r) { return r.Tier === tier; }).length;
    });
    html += '<div class="group-section"><div class="group-head"><span class="tier-dot" style="background:' + TIER_META[tier].bg + ';"></span>' +
      '<span class="group-name">' + TIER_META[tier].short + '</span><span class="group-total">' + (isG1 ? fmtNum(groupTotal) + ' objects' : pvLocked()) + '</span></div>';
    WORKLOAD_DEFS.forEach(function (wd) {
      var rows = DATA.workloads[wd.key] || [];
      var count = rows.filter(function (r) { return r.Tier === tier; }).length;
      var pct = rows.length ? (100 * count / rows.length) : 0;
      html += '<div class="group-wd-row"><span class="wd-name">' + esc(wd.label) + '</span>' +
        '<span class="wd-bar-track"><span class="wd-bar-fill" style="width:' + (isG1 ? pct : 0) + '%;background:' + TIER_META[tier].bg + ';"></span></span>' +
        '<span class="wd-count">' + (isG1 ? fmtNum(count) : pvLocked()) + '</span></div>';
    });
    html += '</div>';
  });
  html += '</div>';
...
// but there is no slider-drag path here at all (everything is disabled), so
// unlike the Full build there is no separate "live" recompute function -
// this always does a single full render.
function renderGroupsTab() {
  var html = '<div class="controls-panel-grid">' +
    '<div id="controls-sliders-wrap">' + buildControlsSliders() + '</div>' +
    '<div id="impact-preview-wrap">' + buildGroupBreakdownPreview() + '</div>' +
    '</div>' +
...
function renderRecoveryTab() {
  ...
  html += '<p class="pill-note">ABR (Autonomous Business Recovery, sequenced by group) vs. Mass Recovery (undifferentiated, no prioritization) - time and downtime cost, for Group 1. SP/OD throughput tier: ' + pvLocked() + ' (auto-selected from this tenant\'s object counts unless overridden above - shown in full in the complete assessment). ABR is the only approach where group size and RTO targets actually determine WHEN priority data comes back. See the Methodology tab for the full derivation.</p>';
  ...
  groupTiers.forEach(function (g, idx) {
    ...
    // with nothing left to place. Badge + inline note instead of an
    // unexplained duplicate number.
    // PREVIEW-ONLY (v3.7.5): only ever surfaced for Group 1 - whether Group
    // 2/3 is empty is itself real information about this tenant, so it's
    // withheld the same as every other Group 2/3 figure below.
    var isEmpty = objCount === 0;
    var emptyBadge = (isEmpty && idx === 0) ? ' <span class="badge empty-tier">Empty</span>' : "";
    var emptyNote = (isEmpty && idx === 0)
    ...
    // responsible to a separate bucket. PREVIEW-ONLY (v3.7.5): Group 1 only,
    // same reasoning as isEmpty above - whether Groups 2/3 exceed their own
    // target is real tenant data.
    var exceedsBadge = (m.exceedsTarget && idx === 0) ? ' <span class="badge exceeds-target">Exceeds target</span>' : "";
    var exceedsNote = (m.exceedsTarget && idx === 0)
    ...
    // PREVIEW-ONLY (v3.7.5, hardened from v3.7.2): Groups 2/3 used to still
    // show their real Mass Recovery time/cost here ("the scale of the
    // problem") - per feedback 2026-07-31, that's now locked too, so nothing
    // beyond Group 1 is legible anywhere in this build, on screen or via
    // view-source (pvLocked() never puts the real number in the DOM).
    var lockedGroupHtml = idx === 0 ? "" :
      '<div class="rt-compare-grid">' +
        '<div class="rt-compare-card mass" style="grid-column:1 / -1;"><div class="rt-label">Mass Recovery - expected wait</div><div class="rt-time">' + pvLocked() + '</div><div class="rt-cost">Downtime cost: ' + pvLocked() + "</div></div>" +
      "</div>" +
  ...
  // Total section - every object, all groups plus Group 4. PREVIEW-ONLY
  // (v3.7.6, relaxed from v3.7.5 per feedback 2026-07-31): the whole-tenant
  // aggregate (object count, ABR-everything time/cost, Mass Recovery-
  // everything time/cost) is real again - it's a single number for the
  // ENTIRE tenant and doesn't reveal how it's split across Groups 2/3/4
  // individually (that per-group split is what stayed locked in the
  // sections above).
  var lastTierName = TIER_ORDER[TIER_ORDER.length - 1];
  var lastM = model.milestones[lastTierName];
  var totalObjCount = lastM.sp.objectCount + lastM.od.objectCount + lastM.ex.objectCount;
  var totalAbrCost = fmtHours(lastM.wallClockCumMin) * state.recovery.costPerHour;
  var totalMassCost = fmtHours(model.massRecoveryMin) * state.recovery.costPerHour;
  ...
  html += '<div class="rt-group-section total">' +
    ...
    '<div class="rt-compare-grid">' +
      '<div class="rt-compare-card abr"><div class="rt-label">ABR - everything online in</div><div class="rt-time">' + fmtMin(lastM.wallClockCumMin) + '</div><div class="rt-cost">Downtime cost: ' + fmtMoney(totalAbrCost) + "</div></div>" +
      '<div class="rt-compare-card mass"><div class="rt-label">Mass Recovery - everything online in</div><div class="rt-time">' + fmtMin(model.massRecoveryMin) + '</div><div class="rt-cost">Downtime cost: ' + fmtMoney(totalMassCost) + "</div></div>" +
    "</div>" +
    '<div class="rt-total-note">The entire value of ABR is in the Group 1 section above: it determines <b>when</b> your most critical data comes back, well ahead of the rest of the tenant.</div>' +
    "</div>";
...
// bootstrap (see recomputeAll). Mirrors the Full script exactly: none of
// these fields are identity data, so Protect-PreviewData never redacts
// them (see that function's $identityFields list) - the aggregate totals
// below are real for every row, same as the existing Group 4 dormant
// callout. Archive Mailbox STORAGE and Recoverable Items (both size and
// count) require a separate Exchange Online connection and a per-mailbox
// loop - see -DetailedSizing (Full-only) - and show a "not collected" note
// until that switch is available. Archive Mailbox COUNT is free (same
// usage report) and always shown.
...
function buildSizingExchangeTableHtml(s) {
  ...
  if (detailed && DATA.meta.sizing) {
    ...
    // Simplified to a bare "*" per skipped cell; a single footnote below the
    // table explains the asterisk, rather than repeating explanatory text
    // in every cell.
    archiveStorageCell = "*";
    archiveItemsCell = "*";
    archiveAvgCell = "*";
...
// NEW v3.10.1: takes the already-computed recovery `model` (matches the Full
// script's signature) so byWorkload can carry each workload's Mass Recovery
// time alongside its Group 4 count/pct. Note this is PREVIEW - the per-
// workload breakdown (including this new field) is computed here exactly
// like the Full script for parity, but rendering-side, both the live report
// (pvLockedPanel) and the PDF export lock the whole "Group 4 Detail" section
// to a "see the full assessment" note - byWorkload's real numbers never
// reach the DOM here, same as before this change.
...
// PREVIEW-ONLY (v3.7.5): a locked placeholder for PDF export - plain text,
// not the blurred on-screen pvLocked(), since a static printed page can't
// blur anything and this needs to read cleanly on paper either way.
function prLocked() { return "Full assessment only"; }
...
// PREVIEW-ONLY (v3.7.5): only the Group 1 column (and no grand Total column)
// is real - every other cell used to show real cross-workload/tenant totals.
function buildGroupOverviewTableHtml() {
  ...
  WORKLOAD_DEFS.forEach(function (wd) {
    ...
    TIER_ORDER.forEach(function (t) {
      rowHtml += '<td class="num">' + (t === group1Tier ? fmtNum(g1Count) : prLocked()) + "</td>";
    });
    rowHtml += "</tr>";
    html += rowHtml;
  });
  html += '<tr class="total-row"><td>Total</td>' +
    TIER_ORDER.map(function (t) { return '<td class="num">' + (t === group1Tier ? fmtNum(grandGroup1Total) : prLocked()) + "</td>"; }).join("") +
    "</tr>";
  html += "</tbody></table>";
  return html;
}
...
function buildAssumptionsHtml() {
  ...
  html += "<tr><td>SP/OD throughput tier (auto-selected unless overridden)</td><td class=\"num\">" + prLocked() + " (requested: " + esc(state.recovery.licenseTier) + ")</td></tr>";
...
// PREVIEW-ONLY (v3.7.5): this used to recompute every whole-tenant/Groups-2/3
// figure independently from DATA/computeRecoveryModel(), the same as the
// live Report tab did before it was locked down - meaning the PDF export
// would have kept leaking real numbers even after the on-screen fix. Group 1
// stays real throughout; everything else is prLocked().
function buildPrintSummaryHtml() {
  var model = computeRecoveryModel();
  var g1 = model.milestones["Critical Group 1"];
  var abrCostG1 = fmtHours(g1.wallClockCumMin) * state.recovery.costPerHour;
  // PREVIEW-ONLY (v3.7.6, relaxed from v3.7.5 per feedback 2026-07-31): the
  // whole-tenant Mass Recovery baseline is a single aggregate figure for the
  // full-tenant comparison against Group 1 - it's real again, same as the
  // on-screen Exec Summary/Report tabs. Groups 1-2/1-3 (Group 2/3-specific
  // scale) stay locked below.
  var fullExposureMoney = fmtHours(model.fullRestoreMin) * state.recovery.costPerHour;
  var g1SavingsMoney = Math.max(0, fullExposureMoney - abrCostG1);
  var pctReduction = fullExposureMoney > 0 ? Math.round((g1SavingsMoney / fullExposureMoney) * 100) : 0;
  ...
  html += '<div class="pr-tiles">' +
    ...
    '<div class="pr-tile"><div class="pr-tile-label">Downtime Cost Avoided</div><div class="pr-tile-value">' + fmtMoney(g1SavingsMoney) + '</div><div class="pr-tile-sub">' + pctReduction + '% reduction vs. a traditional, random-order restore</div></div>' +
  "</div>";
  ...
  html += "<tr><td>Group 1 online</td><td class=\"num\">" + fmtMin(g1.wallClockCumMin) + "</td><td class=\"num\">" + state.recovery.group1Hours + " hr</td><td>" + (g1.exceedsTarget ? '<span class="pr-badge exceeds">Exceeds target</span>' : "Within target") + "</td></tr>";
  ["Groups 1-2 online", "Groups 1-3 online"].forEach(function (label) {
    html += "<tr><td>" + label + "</td><td class=\"num\">" + prLocked() + "</td><td class=\"num\">" + prLocked() + "</td><td>" + prLocked() + "</td></tr>";
  });
  ...
  if (state.recovery.costPerHour > 0 && model.fullRestoreMin > 0) {
    ...
    html += "<tr><td>Group 1 online</td><td class=\"num\">" + fmtMin(g1.wallClockCumMin) + "</td><td class=\"num\">" + fmtMoney(abrCostG1) + "</td><td class=\"num\" colspan=\"2\">" + prLocked() + "</td></tr>";
    ["Groups 1-2 online", "Groups 1-3 online"].forEach(function (label) {
      html += "<tr><td>" + label + "</td><td class=\"num\">" + prLocked() + "</td><td class=\"num\">" + prLocked() + "</td><td class=\"num\">" + prLocked() + "</td><td class=\"num\">" + prLocked() + "</td></tr>";
    });
    html += "<tr><td>Full tenant (Mass Recovery baseline)</td><td class=\"num\">" + fmtMin(model.fullRestoreMin) + "</td><td class=\"num\">" + fmtMoney(fullExposureMoney) + "</td><td class=\"num\" colspan=\"2\">Baseline - no prioritization applied</td></tr>";
    html += "</tbody></table>";
  }
  ...
  html += '<div class="pr-footer">Recovery window: ' + state.recovery.windowDays + " days &middot; Downtime cost: " + (state.recovery.costPerHour > 0 ? fmtMoney(state.recovery.costPerHour) + "/hour" : "not set") + ". Full per-group breakdown, Group 4 detail, and every assumption behind these numbers: see the full assessment.</div>";
...
// detail).
// PREVIEW-ONLY (v3.7.6, relaxed from v3.7.5 per feedback 2026-07-31): the
// whole-tenant Mass Recovery baseline (single aggregate figure vs. Group 1)
// is real again, matching the on-screen Exec Summary/Recovery tabs - Groups
// 2/3-specific figures and Group 4 detail stay prLocked().
// computeDormantSummary() is still called (nothing downstream of it uses the
// real numbers anymore) so the function itself doesn't need touching.
...
// that export is deliberately terse and sizing is a distinct data domain
// from the recovery-time/cost story it tells. Mirrors the Full script
// exactly - built only from aggregate metrics, nothing identity-bearing,
// so no PREVIEW-ONLY redaction is needed.
...
function buildPrintFullHtml() {
  ...
  html += '<div class="pr-subnote">See the full assessment for the complete group-by-group sequencing across your whole environment. ABR does what any puzzle solver does first &ndash; corners and edges, the pieces that actually give you something to work with &ndash; prioritizing recently accessed, modified, and created data so the business is operational in ' + fmtMin(g1.wallClockCumMin) + ".</div>";
  if (state.recovery.costPerHour > 0) {
  ...
  html += '<div class="pr-subnote">ABR (sequenced by group) vs. Mass Recovery (undifferentiated, no prioritization). Group 1 is shown in full below and the whole-tenant total follows; Groups 2/3 detail is available in the full assessment.</div>';

  html += '<div class="pr-avoid-break">';
  html += "<h4 style=\"margin:.7rem 0 .1rem;font-size:.85rem;color:var(--navy);\">Group 1" + (g1.exceedsTarget ? '<span class="pr-badge exceeds">Exceeds target</span>' : "") + " - " + fmtNum(g1.sp.objectCount + g1.od.objectCount + g1.ex.objectCount) + " objects</h4>";
  html += '<div class="pr-subnote" style="margin-top:0;">Recover-first tier (target: ' + state.recovery.group1Hours + " hr)</div>";
  html += '<table class="pr-table"><thead><tr><th></th><th class="num">Time</th><th class="num">Downtime Cost</th></tr></thead><tbody>' +
    '<tr><td>ABR - Group 1 online in</td><td class="num">' + fmtMin(g1.wallClockCumMin) + '</td><td class="num">' + fmtMoney(abrCostG1) + "</td></tr>" +
    '<tr><td>Mass Recovery - expected wait</td><td class="num">' + fmtMin(g1.wallClockCumMin) + '</td><td class="num">' + fmtMoney(abrCostG1) + "</td></tr>" +
    "</tbody></table>";
  html += '<div class="pr-note">Group 1 is the first tier - nothing earlier to sequence against, so its ABR time and Mass Recovery time are the same computation by definition.</div>';
  html += buildStaticBreakdownTableHtml(model, "Critical Group 1");
  html += "</div>";

  ["Group 2", "Group 3"].forEach(function (label) {
    html += '<div class="pr-avoid-break">';
    html += "<h4 style=\"margin:.7rem 0 .1rem;font-size:.85rem;color:var(--navy);\">" + label + " - " + prLocked() + "</h4>";
    html += '<div class="pr-note">Object counts, timing, and downtime-cost detail for ' + label + ' are available in the full assessment.</div>';
    html += "</div>";
  });

  // Whole-tenant total - single aggregate figure (real again, v3.7.6); the
  // per-group split that got you there stays locked above.
  var lastTierName = TIER_ORDER[TIER_ORDER.length - 1];
  var lastM = model.milestones[lastTierName];
  var totalObjCount = lastM.sp.objectCount + lastM.od.objectCount + lastM.ex.objectCount;
  var totalAbrCost = fmtHours(lastM.wallClockCumMin) * state.recovery.costPerHour;
  ...
  html += '<table class="pr-table"><thead><tr><th></th><th class="num">Time</th><th class="num">Downtime Cost</th></tr></thead><tbody>' +
    '<tr><td>ABR - everything online in</td><td class="num">' + fmtMin(lastM.wallClockCumMin) + '</td><td class="num">' + fmtMoney(totalAbrCost) + "</td></tr>" +
    '<tr><td>Mass Recovery - everything online in</td><td class="num">' + fmtMin(model.massRecoveryMin) + '</td><td class="num">' + fmtMoney(totalMassCost) + "</td></tr>" +
    "</tbody></table>";
  html += '<div class="pr-note">ABR\'s value is entirely in WHEN each group comes back, not the full-job duration.</div>';
  html += "</div>";
  ...
  if (dormant.totalObjectsAll > 0) {
    html += '<div class="pr-subnote">See the full assessment for the complete zero-activity breakdown, by workload.</div>';
  }
...
<div class="header">
  ...
  <div class="eyebrow">RECOVERY SEQUENCING ASSESSMENT - PREVIEW</div>
  ...
  <!-- PREVIEW-ONLY (v3.7.2): visible banner so nobody mistakes this for the
       ...
       Groups 2/3/4 are redacted and simplified. -->
  <div class="pill-note" style="margin:1rem 3rem 0;max-width:none;background:#EAF1FE;color:var(--navy);font-weight:600;">This is a PREVIEW build against this tenant's real data. Group 1 is shown in full; Groups 2, 3, and 4 show real totals with per-object detail and ABR timing redacted. Connect with your Rubrik team for the complete assessment.</div>
</div>
<div class="tabbar">
...
function New-M365HtmlReport {
    ...
    $dataObject = [ordered]@{
        meta = [ordered]@{
            ...
            # connection and a per-mailbox loop, not yet wired up (planned:
            # -DetailedSizing switch, Full-only). Placeholder false/null
            # keeps the JS's contract stable now.
            detailedSizingRequested      = $false
            sizing                       = $null
        }
        weights = [ordered]@{
...
# (-TierSplit). Protect-PreviewData (below, per workload) only redacts
# identity fields - never the numeric metrics this tiering walk itself runs
# on - so re-tiering on load always sees the same real data. See
# RECOVERY-MODEL-METHODOLOGY.md.
# ---------------------------------------------------------------------------
...
$mailboxes = @(Protect-PreviewData -Data $mailboxes -WorkloadLabel 'Mailbox')
...
$onedrive = @(Protect-PreviewData -Data $onedrive -WorkloadLabel 'OneDrive Account')
...
$teams = @(Protect-PreviewData -Data $teams -WorkloadLabel 'Team')
...
$sharepoint = @(Protect-PreviewData -Data $sharepoint -WorkloadLabel 'SharePoint Site')
...
if ($sharepointExcluded.Count -gt 0) {
    # Excluded sites never go through Add-Tier (no .Tier property), so every
    # row gets redacted here - not just Groups 2/3/4 - since there's no
    # Group-1 exemption to preserve on an untiered list.
    $sharepointExcluded = @(Protect-PreviewData -Data $sharepointExcluded -WorkloadLabel 'Excluded SharePoint Site')
...
report have per-object identity redacted and (Groups 2/3) ABR timing withheld;
weight sliders, keyword tables, and manual overrides are shown read-only/frozen
