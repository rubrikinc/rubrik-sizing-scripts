...
function Test-RowTagConditionMatch {
    param($Row, $Cond)
    if ($Cond.Key -notmatch '^aws:ResourceTag/(.+)$') { return $false }
    $tagKey = $matches[1] -replace '[^a-zA-Z0-9]', '_'
    $prop = $Row.PSObject.Properties[$tagKey]
...
function Test-IsAccessDenied {
    ...
    if ($ex.GetType().Name -match 'AccessDenied') { return $true }
    if ($ex.PSObject.Properties['ErrorCode'] -and $ex.ErrorCode -match 'AccessDenied') { return $true }
    return $false
}
...
function Get-AWSEBSAndAMIInventory {
    ...
    foreach ($image in $images) {
        ...
        # Attribute the AMI to the source instance when discoverable via the
        # `ec2:source-instance-id` tag. Validate the tag value against the EC2
        # instance-id format (`i-<hex>`) before interpolating into an ARN -- a
        # malformed tag would otherwise produce a malformed ARN downstream.
        # Falls back to the AMI ARN if the tag is missing or invalid.
        $sourceArn = $null
        foreach ($tag in @($image.Tags)) {
            if ($tag -and $tag.Key -eq 'ec2:source-instance-id' -and
                $tag.Value -match '^i-[0-9a-f]+$') {
                $sourceArn = "arn:$($partitionId):ec2:$($Region):$($AccountInfo.Account):instance/$($tag.Value)"
                break
            }
        }
        if ([string]::IsNullOrEmpty($sourceArn)) {
            $sourceArn = "arn:$($partitionId):ec2:$($Region):$($AccountInfo.Account):image/$($image.ImageId)"
        }
...
function Get-AWSBackupCosts {
    ...
    $result = Invoke-CECall -Context "Get-AWSBackupCosts" -AccountInfo $AccountInfo -ScriptBlock {
        Get-CECostAndUsage `
...
function Get-AWSSnapshotStorageCosts {
    ...
    $dimResult = Invoke-CECall -Context "Get-AWSSnapshotStorageCosts-Dims" -AccountInfo $AccountInfo -ScriptBlock {
        Get-CEDimensionValue -TimePeriod $TimePeriod -Dimension USAGE_TYPE -Credential $Credential -Region $Region -ErrorAction Stop
    }
    if ($null -eq $dimResult) { return , $rows }

    $allUsageTypes = @()
    foreach ($d in @($dimResult.DimensionValues)) {
    ...
    while ($idx -lt $matched.Count) {
        ...
        $thisCall = $null
        try {
            $thisCall = Invoke-CECall -Context "Get-AWSSnapshotStorageCosts-Cost" -AccountInfo $AccountInfo -ScriptBlock {
                Get-CECostAndUsage -TimePeriod $TimePeriod -Granularity MONTHLY -Metrics $metrics `
                    -GroupBy @(
        ...
        if ($null -eq $thisCall) { return , $rows }   # CE failure already warned inline
...
function Merge-BackupAggregates {
    ...
    foreach ($entry in @($NativeBackupLists)) {
        ...
        if ($null -ne $date -and ($null -eq $state.LatestBackupDate -or $date -gt $state.LatestBackupDate)) {
            $state.LatestBackupDate      = $date
            $state.LatestBackupSizeBytes = $size
        } elseif ($null -eq $state.LatestBackupDate -and $null -eq $date -and $state.LatestBackupSizeBytes -lt $size) {
            # Sources without CreationDate (e.g. DDBPITR) still need to contribute size.
            $state.LatestBackupSizeBytes = $size
        }
    }
  ...
  foreach ($awsRegion in $awsRegions) {
    ...
    if (-not $SkipBackupCapacity) {
      $rpResult = Get-AWSBackupRecoveryPointInventory -Credential $cred -Region $awsRegion `
          -AccountInfo $awsAccountInfo -AccountAlias $awsAccountAlias `
          -TimeoutMinutes $BackupRecoveryPointTimeoutMinutes
      if ($null -ne $rpResult) {
